...
let isValue = isCount.body.firstElementChild.innerHTML;
...
function filterContact(isValue) {
    ...
    customerList.filter(function (data) {
        let statusFilter;
        let matchData = new DOMParser().parseFromString(
            data.values().status,
            "text/html"
        );
...
function updateList() {
    let values_status = document.querySelector("input[name=status]:checked").value;
...
let trlist = table.querySelectorAll(".list tr");

function SearchData() {

    let isstatus = document.getElementById("idStatus").value;
    let pickerVal = document.getElementById("datepicker-range").value;

    let date1 = pickerVal.split(" to ")[0];
    let date2 = pickerVal.split(" to ")[1];

    customerList.filter(function (data) {
        let matchData = new DOMParser().parseFromString(data.values().status, 'text/html');
        let status = matchData.body.firstElementChild.innerHTML;
        let statusFilter;
        let dateFilter;
        ...
        dateFilter = new Date(data.values().date.slice(0, 12)) >= new Date(date1) && new Date(data.values().date.slice(0, 12)) <= new Date(date2);
...
editBtn.addEventListener("click", function (e) {
    document.getElementById("exampleModalLabel").innerHTML = "Edit Customer";
    let editValues = customerList.get({
        id: idField.value,
    });
    Array.from(editValues).forEach(function (x) {
        let isid = new DOMParser().parseFromString(x._values.id, "text/html");
        let selectedid = isid.body.firstElementChild.innerHTML;
...
function refreshCallbacks() {
    Array.from(removeBtns).forEach(function (btn) {
        btn.addEventListener("click", function (e) {
            ...
            let itemValues = customerList.get({
                id: itemId,
            });

            Array.from(itemValues).forEach(function (x) {
                let deleteid = new DOMParser().parseFromString(x._values.id, "text/html");

                let isElem = deleteid.body.firstElementChild;
                let isdeleteid = deleteid.body.firstElementChild.innerHTML;
        ...
        btn.addEventListener("click", function (e) {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (selectedid === itemId) {
                    ...
                    let statusSelec = val.body.firstElementChild.innerHTML;
...
function deleteMultiple() {
    ...
    for (let i = 0; i < items.length; i++) {
        if (items[i].checked === true) {
            let trNode = items[i].parentNode.parentNode.parentNode;
            let id = trNode.querySelector("td a").innerHTML;
            ids_array.push(id);
        }
    }
